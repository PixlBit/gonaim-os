...
| `packages/couple` | نموذج مساحة الاثنين + التحليل + الأنماط + الإيقاع + الانتباه |
| `packages/couple-db` | مخزَن المساحة (ملف أو Postgres) + حسابان + جلسات |
| `packages/oracle` | الطبقة الاختيارية: بريف محدود ومعروض، وبديل حتمي بدونه |
| `apps/ehna-api` | خادم EHNA//OS — دخول، أفعال، صور، تصدير |
| `apps/ehna` | EHNA//OS — عشر شاشات، عربية أولًا |

---

## EHNA//OS — مساحة الاثنين

بنفس المبادئ، ولاثنين بدل واحد:
[`docs/product/EHNA_OS.md`](docs/product/EHNA_OS.md).

منصة خاصة تبدأ من التجهيز للفرح ولا تنتهي عنده: العش وأسعاره · الفلوس
والتحليلات · المواعيد · الذكريات · الأنماط · وما بينكما وحدكما. حسابان
اثنان يُنشآن من سطر الأوامر، ولا تسجيل ولا دعوة ولا ثالث.

```bash
npm run ehna:setup    # الحسابان + سر الجلسات + كشف تجهيز مصري بلا أسعار مخترعة
npm run ehna          # الخادم → http://localhost:8788
npm run ehna:web      # واجهة التطوير → http://localhost:5174
```

ثلاثة أشياء تميّزها عن أي قائمة مهام:

- **تتنبّأ بالضغط قبل وقوعه** — درجة لكل أسبوع من الثمانية القادمة، محسوبة
  على سعتكم المقاسة من سرعتكم الحقيقية، ومعها "أهدى أسبوع" لتحجزوا فيه
  ليلة لأنفسكم.
- **أنماط بدليل** — ستة محاور تُقاس من سلوك مسجَّل، وسبعة إعلانات يكتبها كل
  واحد عن نفسه، والفرق بينهما هو ما لا يراه أحدكما وحده. ولا درجة بلا
  عيّنة كافية: المجهول يظهر مجهولًا لا صفرًا.
- **نبض العلاقة** — نسبة ما كان شخصيًا من نشاطكم. حين تقترب من الصفر تقول
  المنصة ذلك مرة واحدة بهدوء: التجهيز أخذ المساحة كلها.

والطبقة الاختيارية (نموذج Claude) تعمل بمفتاح وتُعطّل بدونه بلا أن يتعطّل
شيء آخر — وقبل أي نداء تعرض **حرفيًا** ما سيخرج من المساحة، وما لا يخرج
منها أبدًا.
