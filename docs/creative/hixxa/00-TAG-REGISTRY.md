...
| `@bag` | Prop | **جسم** الحقيبة كاملًا: الشكل والنِسَب، حبيبة الجلد ولونه، الحبكات، الغطاء، السير ونقطة التقائه بالجسم، الإبزيم وكل قطعة معدنية، والبلى عند الحواف | لا يحدد **موضع** الشنطة (ذلك لمرجع المكان)؛ لا تتضاعف ولا تتحرك بلا لمس | 01، B09، B10، P01، P02 |
...
- **الأجسام الموجودة في مرجع المكان كمان** (`@bag`، `@basket`، `@desk`، اللمبة): `@loc_location-room-1` يصوّرها من بعيد بتفاصيل غير مقروءة. اكتب دائمًا أن التاق الخاص هو **السلطة الوحيدة على الجسم** وأن المكان يعطي **الموضع فقط**، وأن التاق **يفوز** عند التعارض — الصمت هنا ليس حيادًا، بل ترجيح صامت لمرجع المكان. الصيغة الكاملة في الماستر §6E.
...
### 3.7 كرة الورق وعدّاد السلة عبر السلسلة

#### شكل الكرة

الكرة **صغيرة**: 7–8 سم، تُغلق كاملة داخل قبضتها — بحجم برقوقة، لا تفاحة ولا شمّامة ولا كرة قدم. وشكلها **زاويّ لا كروي**: سطوح مضغوطة مسطحة تلتقي عند حروف طيّ حادة، وأركان ناتئة رفضت الانضغاط، ودرزة واحدة ظاهرة حيث أُغلقت آخر طية.

قاعدتان تمنعان أشيع خطأين:

```text
It is never a smooth sphere, never a soft wad of tissue, never a moulded papier-mache lump.
...
الثانية ضرورية لأن النموذج يميل إلى استنساخ الكرة نفسها عند تعدّدها، فتظهر كرتان متطابقتان — وهو ما يكشف التوليد فورًا.

ولون الكرة يتبع الورقة التي صُنعت منها: ورقة جرافيت فقط تُقرأ كريمية سادة، أما ورقة ملوّنة فتُظهر **شظايا صغيرة من لونها على السطوح المتجهة للخارج فقط**، حيث انطوى الوجه المرسوم للخارج.

#### عدّاد السلة

السلة عدّاد تصاعدي عبر السلسلة، ونقطة الصفر هي الثعلب:

| المرحلة | عند الفريم صفر | عند النهاية |
|---|---|---|
| **الثعلب — أول رفض** (`B05`) | **فارغة** | كرة واحدة |
| **حلقة التصعيد** (`B03`) | كرة واحدة | ثلاث + رابعة في الهواء |
| المراحل التالية | ما تركته المرحلة السابقة | + عدد الأوراق المستهلكة |

**قاعدة تحرير:** في كل برومبت يوجد بلوك واحد اسمه `BALL COUNT — SEQUENCE POSITION`؛ غيّر رقمه وحده ثم طابق أرقام الدورات داخل الشوت. لا تكتب العدد في مكان ثانٍ.
