# HIXXA — RIBBON, FIRST BREATH

> **المدة:** 30 ثانية — **الشوتات:** 12 — **القطعات:** 11 hard cuts
> **المعيار:** CINEDANCE Bible v2.0 — Dimensional Hand-Painted Cinematic
> **المكان:** غرفة HIXXA، أمام المكتب، أواخر العصر
> **التاقات:** `@loc_location-room-1` `@char_hixxa` `@hixa-face` `@desk` `@cat` `@weaponribbon` `@ribbon`
> **القوس الشعوري:** عرض هادئ → تردد → أول تماس → التفاف → تجسد → ثقة صامتة

**القاعدة الحاكمة للتحول:** لا يوجد وميض ولا انفجار ضوء. التحول **ميكانيكي مادي**: الشريط الأبيض يلتف ويبني، والقط يختفي داخل الشرنقة قبل أن يقف Ribbon. هذا يخدم الـRender Bible ويمنع الـneon glow.

---

```text
################################################################
HIXXA — RIBBON, FIRST BREATH
################################################################

SEEDANCE 2.5 — MULTI-REFERENCE R2V — 30 SECONDS — 16:9 — 24 FPS

CONTROLLED 12-SHOT SEQUENCE — EXACTLY 11 DIRECT HARD CUTS
REAL-TIME MOTION — NO SLOW MOTION — NO SPEED RAMPS — NO VISUAL TRANSITIONS

REFERENCE ASSIGNMENTS

@loc_location-room-1 controls the exact room architecture, permanent geography,
materials and the real window beside the desk. It does not control the camera angle.

@char_hixxa controls HIXXA's exact single full-body identity, skin tone,
body proportions, curly hairstyle and established wardrobe.
It does not control the camera angle, the shot size or her moment-to-moment expression.

@hixa-face reinforces only the face of that same single HIXXA.
It never creates a second person.

@desk controls the exact desk surface, wood grain and permanent tools.
It does not introduce any additional device or interface.

@cat controls the exact cat identity, proportions, markings and collar.
It never appears as a reference sheet or drawn page inside the world.

@weaponribbon controls the exact slim black shaft and its one continuous white ribbon:
length, thickness, ribbon width and the metal collar near the tip.
Exactly one ribbon exists and it never splits, doubles or becomes a blade.

@ribbon controls the exact transformed form: white enamelled plates, folded wing shapes,
proportions and the way the black shaft sits in its grip.
It appears only after the transformation shot and never stands beside the cat.

HANDS AND PAINT STAINS

Both of HIXXA's hands carry permanent dried paint stains from long working days, and those
stains are part of her identity in every shot: soft pale-blue and cyan patches worn into the
sides of the thumb and across the finger pads, a smaller lilac or dusty-pink patch near one or
two nail beds, scattered cream and white flecks over the knuckles and the backs of the
fingers, and faint colour caught in the cuticle edges.

They read as dried, absorbed and worn — thin, matte, slightly faded, and following the skin's
own creases and folds. They are never wet paint, never fresh droplets, never glossy, raised,
wet-looking or glowing. The stains sit beneath the skin's hand-painted shading rather than on
top of it, so the hand keeps its full sculptural volume, knuckle relief and tendon structure.

The nails stay clean and natural with a visible pale nail plate; colour gathers around the
nail edges and cuticles, not across the nail surface.

The pattern is soft and irregular and never resolves into a graphic shape, a logo, a glove, a
tattoo, a bruise, a wound or dirt. Its placement, colours and density stay consistent across
every shot, lens and angle, and it never washes off, spreads or changes colour within a film.

PROP SCALE LOCK

HIXXA stands about 165 centimetres tall and her hand is the scale reference for this entire
film. Every object below is locked to both a real dimension and a hand-relative anchor.
Where the two seem to disagree, the hand-relative anchor wins. No object is rendered larger or
smaller than its stated relationship to her hand or body, and no object changes size between
shots, lenses or angles.

BLACK SHAFT AND RIBBON — the shaft is about 90 centimetres long and 2 across, with one
continuous white ribbon roughly 4 centimetres wide and 3 metres long.

SMALL RED PENCIL IN HER CURLS — a short stub about 9 centimetres long and pencil-thin,
roughly the length of her index finger. It never becomes a full-length pencil, brush or rod.

DESK LAMP — about 40 centimetres tall on the desk, its shade roughly the size of her cupped
hand.

CREATURE SCALE — the cat stands about 30 centimetres at the shoulder. Ribbon stands about 110 centimetres, clearly shorter than HIXXA and never taller than her shoulder.

CONTINUITY MAP

World geography: the bed remains world-left, the desk and the real window remain world-right,
the door remains on the rear wall. There is no mirror in this room.
Never mirror, rotate or rebuild this geography.

HIXXA begins standing at the desk, world-left of it, facing world-right,
weight on her anatomical left foot, the black shaft held horizontally in both hands,
one small red pencil in her curls. The cat sits on the desk, watching her hands.

The shaft progresses through one continuous state:
[held in both her hands] → [laid flat on the desk] → [ribbon tail hanging off the edge]
→ [under the cat's paw] → [wound around the cat] → [held in Ribbon's grip].

The cat progresses through one continuous state:
[seated, watching] → [standing, one paw lifted] → [paw on the shaft]
→ [wrapped and hidden inside the ribbon] → [replaced by Ribbon standing].

The cat and Ribbon are never visible in the same frame.
Each shot begins at the exact next physical instant inherited from the previous shot.
No repeated action, pose reset, teleportation, wardrobe reset or unexplained time jump.

################################################################
TIMELINE
################################################################

SHOT 01 — 0.00–2.60
35mm DESK-SIDE WIDE, camera world-left, window beyond the desk.

Frame zero already shows HIXXA standing with the black shaft level in both hands
and the cat seated on the desk facing her. She lowers the shaft onto the wood in one
controlled movement, releasing with her anatomical right hand last. Her shoulders stay down.

Small retreat as the shaft comes to rest.
End with the shaft flat on the desk and both her hands clear of it.

HARD CUT ON THE RELEASE.

SHOT 02 — 2.60–4.40
90mm MACRO INSERT along the desk surface.

Inherit the shaft settling. It rocks once against the grain and stops. The white ribbon
runs off the end and its tail slides over the desk edge under its own weight,
swinging twice before it hangs still.

Hold fixed.
End with the ribbon tail hanging vertical and motionless.

HARD CUT ON THE RIBBON GOING STILL.

SHOT 03 — 4.40–6.60
70mm LOW, desk surface near lens height, cat in frame.

Inherit the still ribbon. The cat rises from sitting, walks two steps along the wood
and stops short of the shaft. Its front right paw lifts and hovers, claws in, undecided.
Its shadow falls across the black shaft.

Extremely subtle forward drift.
End with the paw held in the air a few centimetres above the shaft.

HARD CUT ON THE PAW HOLDING.

SHOT 04 — 6.60–8.80
50mm MEDIUM, HIXXA's hands at the edge of frame, cat centre.

Inherit the hovering paw. She keeps her hands still and does not reach in.
The paw comes down on the shaft. The wood beneath takes the small weight,
and the whole length of the ribbon twitches once from collar to tail.

Hold fixed.
End with the paw resting on the shaft and the ribbon tail lifted mid-twitch.

HARD CUT ON CONTACT.

SHOT 05 — 8.80–11.20
90mm MACRO INSERT on the paw and the ribbon.

Inherit the lifted tail. The ribbon climbs the cat's foreleg in one continuous winding
motion, laying itself flat against the fur with real cloth tension. It does not glow.
It draws snug and the foreleg takes the pressure.

Slow track upward along the winding.
The macro perspective preserves real surface relief and tactile separation between woven ribbon,
fur and the matte shaft beneath them.

End with the ribbon wound to the shoulder and still climbing.

HARD CUT ON THE RIBBON PASSING THE SHOULDER.

SHOT 06 — 11.20–14.00
40mm MEDIUM WIDE, desk and window in frame.

Inherit the ribbon at the shoulder. It spirals fast around the whole cat and closes into a
tight vertical cocoon standing on the desk. The black shaft is drawn upright into its centre.
White enamelled plates form out of the folds, layer over layer, like fabric setting into shell.
No flash, no light burst, no particles.

Slow tilt up with the growing cocoon.
The dimensional tilt keeps the cocoon a solid volume in the room rather than a shape on a plane:
it occludes the desk behind it and its contact shadow travels with it.

End with the cocoon fully closed, standing upright, the shaft vertical at its core.

HARD CUT ON THE COCOON CLOSING.

SHOT 07 — 14.00–16.80
32mm THREE-QUARTER from the door side, room depth behind.

Inherit the closed cocoon. It splits along one seam and opens outward. Ribbon stands up
inside it, taking its own weight on the desk for the first time, wing shapes unfolding
once and then folding back down against its back. The shaft ends up in its right grip.
The desk wood takes the load with a low creak.

Hold fixed.
End with Ribbon standing, wings folded, shaft held at its side.

HARD CUT ON THE WINGS FOLDING.

SHOT 08 — 16.80–19.20
85mm PORTRAIT of HIXXA, warm window light on her cheek.

Inherit her position at the desk. She takes half a step back on her anatomical right foot,
her weight transfers and her breath catches once. Her eyes widen a few millimetres,
then steady. Her hands stay open at her sides.

Extremely subtle retreat.
Keep the face fully dimensional through stable cheek volume, nose projection, jaw structure,
eyelid depth and natural light wrapping.

End with her weight settled on the back foot and her gaze level and steady.

HARD CUT ON HER WEIGHT SETTLING.

SHOT 09 — 19.20–21.80
70mm PROFILE CLOSE on Ribbon, camera world-left.

Inherit Ribbon standing with the shaft at its side. It rotates the shaft once through a slow
half turn, the ribbon following a full beat behind it and describing a soft loop in the air.
The plates at its shoulder shift against each other with a dry ceramic sound.

Hold fixed.
End with the shaft vertical again and the ribbon still closing its loop.

HARD CUT ON THE RIBBON COMPLETING THE LOOP.

SHOT 10 — 21.80–24.60
50mm MEDIUM, desk-level, both Ribbon and her forearm in frame.

Inherit the vertical shaft. Ribbon plants the shaft's end on the desk. The wood answers
with a short knock, the ribbon settles down along the shaft and hangs. Loose pages on the
desk lift and drop under the small displaced air.

Hold fixed.
End with the shaft planted, the ribbon hanging still and the pages back down.

HARD CUT ON THE PLANT.

SHOT 11 — 24.60–27.40
85mm CLOSE TWO-SHOT, her palm entering from frame edge.

Inherit the planted shaft. She raises her anatomical right hand slowly, palm up,
and holds it in the air without closing the distance. Ribbon tips the head of the shaft
toward her palm, stopping a hand's width short. Neither touches.

Extremely subtle forward drift.
End with the shaft head angled toward her open palm and both of them still.

HARD CUT ON THE SHAFT REACHING ITS ANGLE.

FINAL SHOT — 27.40–30.00
28mm ROOM WIDE from the door side, warm window world-right.

Inherit her raised palm and the angled shaft. She lowers her hand to her side.
Ribbon straightens the shaft back to vertical. Both stand at the desk in the same
warm light, neither moving toward the other.

End on a locked stable frame: Ribbon standing on the desk with one shaft planted and one
ribbon hanging, HIXXA standing world-left with her hands at her sides, the red pencil in
her curls, the bed and satchel undisturbed world-left and the warm window unchanged.

################################################################
PHYSICS AND PERFORMANCE
################################################################

All movement is grounded and human: real foot contact, believable balance,
shoulder contribution, joint compression, natural recovery and restrained micro-motion.

The ribbon behaves as one continuous length of real cloth: it has tension when pulled,
it lags behind the shaft, it loops rather than snapping, and it never becomes rigid.
The white plates are hard shell with weight; they knock and slide, they do not stretch.
The desk takes real load and answers with creak and knock.
Ribbon's wing shapes have mass and fold along fixed joints.

No object moves before direct contact or a visible physical force.
The transformation is a mechanical winding event, not a light event.

################################################################
MOOD AND RENDER CONTRACT
################################################################

Preserve the established HIXXA dimensional hand-painted cinematic animation language.

The image is fully constructed in three-dimensional space: volumetric characters,
modeled facial planes, rounded body forms, dimensional architecture, real perspective,
physical occlusion, natural parallax and clear foreground-to-background separation.

Render every character and object with convincing sculptural volume comparable to
high-end 3D animation, while treating every visible surface through controlled
hand-painted illustration: clean illustrated shapes, softly brushed shading,
painterly color transitions, subtly drawn edges, tactile material variation and
restrained authored texture.

Skin, curls, clothing fabric, desk wood, the ribbon's woven cloth, the black shaft's
matte finish and Ribbon's enamelled shell plates remain materially distinct and physically
dimensional.

The result feels like a richly hand-painted cinematic frame occupying real
three-dimensional space: never flat 2D, never a paper cutout, never photoreal live
action and never glossy plastic or toy-like CGI.

Maintain stable facial volume, eye size, curl mass, body proportions, wardrobe
construction and material response across every lens and angle.

Ribbon is a fully sculptural body: overlapping plates with real thickness, shadowed gaps
between them and hand-painted shading that follows their curvature.

################################################################
LIGHTING AND VISUAL CONTINUITY
################################################################

The real window beside the desk remains the dominant warm late-afternoon source, world-right.
Its world direction, color temperature, exposure, shadow placement and time of day remain
unchanged across every angle. Warm directional light wraps her curls, cheek planes, hands and
clothing folds, and rakes across the desk wood so its grain reads as relief rather than pattern.
Ribbon's white plates take warm light on the window side and hold soft shaped shadow on the
other, so its volume reads clearly without blowing out to flat white.

Use soft directional shadow, stable contact shadows and restrained atmospheric
perspective to preserve sculptural volume and real dimensional depth.

Colors remain rich and controlled without neon glow or excessive saturation.

No flat 2D or paper-cutout motion. No photoreal live action. No glossy plastic or
toy-like CGI. No waxy skin, generic game-render materials, watercolor bleed,
unmotivated neon lighting, heavy grain or style drift between shots.
No glow, no light burst and no particle sparkle at any point of the transformation:
it is a mechanical winding event lit by the same window as everything else.

################################################################
AUDIO
################################################################

No dialogue, narration, lyrics, captions or subtitles.

0.00–4.40: room tone, window air, the shaft laid down on wood, the ribbon tail sliding.
4.40–8.80: soft paw steps on desk wood, then one small contact.
8.80–14.00: continuous cloth winding, tightening tension, then a fast wrapping rush
that ends abruptly as the cocoon closes.
14.00–19.20: one dry seam split, ceramic plate movement, a low desk creak, her caught breath.
19.20–24.60: the ribbon passing through air, a short knock as the shaft is planted, pages settling.
24.60–30.00: room tone only, two quiet breaths.

Every sound begins only after its visible physical cause.

################################################################
PRESERVATION LOCKS
################################################################

Only one HIXXA appears.
Preserve her exact face, body proportions, skin tone, hair mass, wardrobe and anatomy. Preserve the permanent dried paint stains on both of her hands exactly as established: same placement, same colours, same density, dried and matte, in every shot and at every lens. Every object holds the exact size given in the prop scale lock, measured against her hand, in every shot and at every lens.

Exactly one cat before the transformation and exactly one Ribbon after it.
The cat and Ribbon never appear in the same frame and the cat never reappears.
Exactly one black shaft and one continuous white ribbon at all times.

Ribbon does not fly, does not leave the desk and does not attack anything.
No second creature enters the room.

The room contains one real window beyond the desk.
Do not replace it with a mirror and do not show a reflected second HIXXA.

Every angle change is a clean direct hard editorial cut.
No fade, dissolve, wipe, morph, flash, blur transition, whip transition,
animated overlay or generated interstitial frame.
```

---

## فحص الاستمرارية

| القطعة | آخر فريم | أول فريم بعدها |
|---|---|---|
| 01→02 | العصا مستوية واليدان بعيدتان | نفس الوضع من ماكرو |
| 03→04 | الكف معلق فوق العصا | نفس التعليق ثم النزول |
| 06→07 | الشرنقة مغلقة والعصا رأسية بداخلها | نفس الشرنقة تنشق |
| 09→10 | العصا رأسية والشريط ينهي اللفة | نفس الوضع قبل الغرز |
| 11→12 | رأس العصا مائل نحو كفها | نفس الميل ثم العودة |

**مجموع الأزمنة:** 2.60 + 1.80 + 2.20 + 2.20 + 2.40 + 2.80 + 2.80 + 2.40 + 2.60 + 2.80 + 2.80 + 2.60 = **30.00**
