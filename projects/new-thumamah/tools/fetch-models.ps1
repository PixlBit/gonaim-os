# ينزّل نماذج نيو ثمامة بأسمائها الصحيحة. شغّله في PowerShell من جذر المشروع:
...
foreach ($k in $Files.Keys) {
  ...
  try {
    Invoke-WebRequest -Uri "$B/$($Files[$k])" -OutFile $dest -UseBasicParsing
    $kb = [math]::Round((Get-Item $dest).Length / 1KB)
    Write-Host ("{0,-24} تم  ({1} ك.ب)" -f $k, $kb)
  } catch {
    Write-Host ("{0,-24} فشل" -f $k)
  }
}
