...
/**
 * عزل نصّ لا نعرف اتجاهه داخل جملة.
 *
 * هذه المنصة تضع أسماءً كتبها أصحابها داخل جمل تولّدها هي: «نور كاتباها من
 * ٥ شهور»، و«Booked — كتب الكتاب». والاسم قد يكون بأي اتجاه، والجملة
 * بالاتجاه الآخر. وخوارزمية البايدي في المتصفح تعامل النصّ المجاور للاسم
 * على أنه امتداد له، فتقفز الأرقام وعلامات الترقيم إلى الجهة الخطأ:
 * «كتب الكتاب — 18 December 2026» تُرسم «18 — كتب الكتاب December 2026».
 *
 * والحلّ ليس `dir` على عنصر — فأغلب هذه النصوص سلاسل لا عناصر — بل محرفا
 * العزل في يونيكود: FSI يفتح جزيرة يُحسب اتجاهها من محتواها وحدها، وPDI
 * يغلقها. وهما محرفان بلا رسم: لا يظهران ولا يشغلان عرضًا، ويُنسخان مع
 * النصّ بلا أثر.
 *
 * القاعدة: كل ما كتبه إنسان ودخل جملة كتبتها المنصة يمرّ من هنا.
 */
export function bdi(text: string): string {
  return `\u2068${text}\u2069`;
}
